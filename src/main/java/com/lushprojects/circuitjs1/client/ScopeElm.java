...
class ScopeElm extends CircuitElm {
    ...
    public void setScopeRect() {
        int i1 = simUi.transformX(min(x, x2));
        int i2 = simUi.transformX(max(x, x2));
        int j1 = simUi.transformY(min(y, y2));
        int j2 = simUi.transformY(max(y, y2));
    ...
    void draw(Graphics g) {
        ...
        g.context.scale(1 / simUi.transform[0], 1 / simUi.transform[3]);
        g.context.translate(-simUi.transform[4], -simUi.transform[5]);
