...
public class CCVSElm extends VCCSElm {
    ...
    public CCVSElm(CircuitDocument circuitDocument, int xa, int ya, int xb, int yb, int f,
                   StringTokenizer st) {
...
//	    parseExpr();
    ...
    public CCVSElm(CircuitDocument circuitDocument, int xx, int yy) {
    ...
    public void doStep() {
        ...
        if (expr != null) {
            ...
            for (i = 0; i != inputPairCount; i++) {
                ...
                if (Math.abs(dx) < 1e-6)
                    dx = sign(dx, 1e-6);
				simulator.stampMatrix(vno, vni, -dx);
                // adjust right side
                rs -= dx * cur;
			...
			simulator.stampRightSide(vno, rs);
        }
