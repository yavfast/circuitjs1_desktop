...
public class OpAmpRealElm extends CompositeElm {
    ...
    public void draw(Graphics g) {
        ...
        drawDots(g, geom().getLead2(), geom().getPoint2(), curCounts[2]);
    ...
    public void setPoints() {
        ...
        interpPoint2(geom().getPoint1(), geom().getPoint2(), in1p[0], in2p[0], 0, hsswap);
        interpPoint2(geom().getLead1(), geom().getLead2(), in1p[1], in2p[1], 0, hsswap);
        interpPoint2(geom().getLead1(), geom().getLead2(), textp[0], textp[1], .2, hsswap);

        // position rails; ideally in middle, but may need to be off-center to fit grid
        double railPos = .5 - ((dn / 2) % circuitEditor().gridSize) / (ww * 2);
        interpPoint2(geom().getLead1(), geom().getLead2(), rail1p[1], rail2p[1], railPos, hs * 2 * (1 - railPos));
        interpPoint2(geom().getLead1(), geom().getLead2(), rail1p[0], rail2p[0], railPos, hs * 2);

        Point tris[] = newPointArray(2);
        interpPoint2(geom().getLead1(), geom().getLead2(), tris[0], tris[1], 0, hs * 2);
        triangle = createPolygon(tris[0], tris[1], geom().getLead2());
        plusFont = new Font("SansSerif", 0, 14);
        setPost(0, in1p[0]);
        setPost(1, in2p[0]);
        setPost(2, geom().getPoint2());
        setPost(3, rail1p[0]);
        setPost(4, rail2p[0]);
    }
