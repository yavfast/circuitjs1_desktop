...
public class VCCSElm extends ChipElm {
    ...
    public VCCSElm(CircuitDocument circuitDocument, int xa, int ya, int xb, int yb, int f,
                   StringTokenizer st) {
    ...
    public VCCSElm(CircuitDocument circuitDocument, int xx, int yy) {
    ...
    double getConvergeLimit() {
        ...
        if (simulator().subIterations < 10)
            return .001;
        if (simulator().subIterations < 200)
            return .01;
        return .1;
    }
...
//        	    sim.console("vcvs " + i + " " + getNodeVoltage(i) + " " + lastVolts[i] + " " + sim.subIterations);
            }
...
//        	if (Math.abs(getNodeVoltage(inputCount)-v0) > Math.abs(v0)*.01 && sim.subIterations < 100)
//        	    sim.converged = false;
            double rs = v0;
            ...
            for (i = 0; i != inputCount; i++) {
                ...
                if (Math.abs(dx) < 1e-6)
                    dx = sign(dx, 1e-6);
                simulator().stampVCCurrentSource(getNode(inputCount), getNode(inputCount + 1), getNode(i), 0, dx);
...
//        	sim.console("ccers " + rs);
            simulator().stampCurrentSource(getNode(inputCount), getNode(inputCount + 1), rs);
