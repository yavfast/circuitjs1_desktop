...
### Візуальна регресійна перевірка (DevMode + JS API)
- Додано окремий список елементів, що фейлять/пропускаються під час серійного імпорту: `ai_memory/geom_visual_checks/failing_elements.md`.
- TODO: `Optocoupler` — тимчасово **SKIP** (JSON import/finalize/setPoints не завершується успішно); повернутись до фіксу після завершення покриття решти типів.

### Зміна стратегії (2025-12-25): **FORCED / HARD MODE**
За запитом: рефакторинг переводиться у форсований режим.

Ключові правила (must):
- **Поля геометрії в `CircuitElm` стають недоступними** (видаляються/закоментуються/замінюються так, щоб їх неможливо було використовувати напряму).
- Будь-яке читання/запис геометрії має йти **через `geom()` / `ElmGeometry`**.
- До завершення міграції **всіх** елементів компіляція **може і буде** неуспішною (це очікувано і є частиною контролю).
- Для перевірки еквівалентності логіки дозволено/рекомендовано використовувати `git diff` (і локальні порівняння формул/алгоритмів).
...
> **Оновлення:** це обмеження більше не діє як вимога. У HARD MODE ми **свідомо відмовляємось від зворотної сумісності по полях**, щоб примусити завершити міграцію. Зворотна сумісність може тимчасово зберігатися лише через нові методи/`geom()` API, але не через прямі поля.
...
Нижче — оновлений порядок робіт під **FORCED / HARD MODE**. На відміну від попередньої версії, проміжні стани можуть **не збиратись**. Мета — зробити “неможливо продовжувати по-старому”, і крок за кроком довести всю кодову базу до `geom()`.

### Важливий контракт HARD MODE
1) **Ніяких геометричних полів у `CircuitElm`** (або вони замінені так, щоб компілятор не дозволяв їх використовувати). До геометрії звертаємось тільки через `geom()`.
2) Будь-яка локальна геометрична математика в елементах має бути або:
    - виконана через методи `ElmGeometry` (стандартизована), або
    - через локальні сніпшоти значень, отриманих з `geom()`/методів (без читання legacy полів).
3) Якщо елементу потрібно “підкрутити derived” — це оформлюється як хук/метод у `ElmGeometry` (або `adjustDerivedGeometry(ElmGeometry)`), без прямого запису в derived поля.
4) `git diff` використовується як інструмент контролю “не змінили формулу/алгоритм” при міграції.

---

### Фаза H0 — Підготовка API в `ElmGeometry` (status: planned)
**Ціль:** перед тим як відрізати поля, забезпечити мінімальний набір API, який дозволяє переписувати елементи без guesswork.

- [ ] Додати в `ElmGeometry` читальні accessor-и/сніпшоти для:
   - endpoints: `getX()/getY()/getX2()/getY2()` або `getPoint1()/getPoint2()` (як canonical)
   - derived: `getDx()/getDy()/getDn()/getDsign()/getDpx1()/getDpy1()`
   - leads: `getLead1()/getLead2()`
   - bbox: `getBoundingBox()` + `setBbox/adjustBbox/initBoundingBox`
- [ ] Додати “стандартні” helper-и, які зараз розкидані по елементах (додавати лише те, що повторюється):
   - геометрія перпендикуляра/offset, `interpPoint*`, axis-aligned derived recompute, clamp minDn, handle points
- [ ] Визначити канонічні методи мутації (і використовувати лише їх): `setEndpoints`, `translate`, `dragTo`, `movePoint`, `flip*`.

**Результат:** елемент можна переписати так, щоб взагалі не залежати від полів `CircuitElm`.

---

### Фаза H1 — “Відрізання полів”: зробити використання полів неможливим (status: planned)
**Ціль:** форсувати міграцію компілятором.

Політика:
- Поля, що стосуються геометрії (`x/y/x2/y2`, `point1/point2`, `lead1/lead2`, `boundingBox`, `dx/dy/dsign/dn/dpx1/dpy1`) мають бути **закоментовані/видалені/перейменовані** так, щоб:
   - компілятор не дозволяв їх читати/писати в підкласах і сторонньому коді;
   - будь-який залишковий доступ давав **compile error**.

Рекомендований механізм (детерміновано, без “магії”):
- [ ] Фізично **прибрати** поля з `CircuitElm`.
- [ ] Надати натомість **методи**, що делегують у `geom()` (або вимагати прямий `geom().get...()`):
   - `geom()` як єдиний шлях доступу
   - за потреби: `getPoint1()/getPoint2()` тощо, але **без** legacy імен полів

**Очікуваний ефект:** проект перестає збиратись, доки не буде переписаний кожен use-site.

---

### Фаза H2 — Серійний повний рефактор елементів (status: planned)
**Ціль:** послідовно пройти всі `*Elm` та перевести їх на `geom()`.

Порядок виконання (щоб зменшити хаос):
1) Базові 2-terminal прості елементи (резистивні/джерела/пасивні) — найменше нестандартної геометрії.
2) Елементи з `flip*/drag/movePoint` overrides.
3) Gate/logic сімейства (високий leverage).
4) Multi-post / складні pins (opamp/transformer/transistor/chip/custom).

Правила переписування одного елемента:
- [ ] Заборонено: читати/писати “старі” поля (вони вже мають бути недоступні).
- [ ] Використовувати `ElmGeometry` як джерело істини: endpoints/derived/points/leads/bbox.
- [ ] Винести повторювані шматки математики у `ElmGeometry` (п.4 вимог користувача).
- [ ] Після міграції елемента — коротко перевірити логіку за допомогою `git diff` (формули/гілки), і лише потім переходити далі.

**Критерій завершення фази:** нема compile errors, повна збірка проходить.

---

### Фаза H3 — Прибирання “перехідних” гачків і стабілізація (status: planned)
**Ціль:** після того як все збирається — прибрати тимчасові допуски.

- [ ] Прибрати будь-які “dirty” мітігації на legacy writes (вони не потрібні, якщо полів немає).
- [ ] Уніфікувати bbox/leads оновлення через `ElmGeometry`.
- [ ] Запустити повний `npm run buildgwt` як критерій зеленої точки.
...
### Фаза E — (скасовано у HARD MODE) Депрекейт/м’яка інкапсуляція
У HARD MODE ця фаза **замінюється** на Фазу H1 (“відрізання полів”).
Soft-deprecation підходить для безперервно зеленого білду, але не відповідає вимозі: “поки не завершиться рефакторинг всіх елементів, компіляція не буде успішною”.
